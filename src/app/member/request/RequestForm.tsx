...
export default function RequestForm({ searchType }: { searchType?: string | null }) {
  const [state, formAction, isPending] = useActionState(
    submitPropertyRequest,
    { success: false }
  );

  const isProxy = searchType === "proxy";
          ...
          {state.success ? (
            ...
            <>
              ...
              <form action={formAction} className="space-y-8">
                ...
                {/* Delegate Info - Only for proxy members */}
                {isProxy && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-5">
                    ...
                    <p className="text-xs text-gray-500 mb-2">
                      代理でお探しのお客様の情報をご記入ください（お名前・ご関係など）
                    </p>
                    <textarea
                      name="delegateInfo"
                      rows={3}
                      className={inputClass + " resize-vertical"}
                      placeholder="例：山田太郎様（顧問先）/ 株式会社○○ 佐藤部長"
                    />
                  </div>
                )}
